    ...
    <section class="dashboard">
        ...
        <div class="dash-content">
            <div class="overview">
                ...
                <?php
                $result = DB::get_all_requests($db);
                $unassigned_requests = [];
                ?>
                <div class="boxes">
                    <div class="box box1" style="box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, 0.2);">
                        <span class="text">Totales</span>
                        <span class="number"><?= $result->rowCount() ?></span>
                    </div>
                    <div class="box box2" style="box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, 0.2);">
                        <span class="text">Asignadas</span>
                        <span class="number">0</span>
                    </div>
                    <div class="box box3" style="box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, 0.2);">
                        <span class="text">Rechazadas</span>
                        <span class="number">0</span>
                    </div>
                </div>
            </div>
                ...
                <table id="request_table" class="display">
                    ...
                    <tbody>
                        ...
                        foreach($result as $row):
                            if($row['assigned'])
                                array_push($unassigned_requests, $row);
                        ...
                        <tr>
                            ...
                            <td>
                                <button id="<?= $row['request_id'] ?>" class="show-request block w-full md:w-auto text-white  focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button" style="background-color: #002B7A;">Ver más</button>
                            </td>
                        </tr>
                        <?php endif;
                            endforeach; ?>
            ...
            <div class="activity">
                ...
                <table id="awating_request_table" class="display">
                    ...
                    <tbody>
                        <?php
                        foreach($unassigned_requests as $row):
                            if($row['assigned']):?>
                        <tr>
                            <td>
                                <?= $row['request_id'] ?>
    ...
    <div class="relative p-4 w-full max-w-4xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow overflow-x-auto">
            ...
            <div class="px-6 space-y-1">
                ...
                <div class="flex">
                    ...
                    <div class="flex-auto px-6">
                        <label for="sel-area" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Area a asignar</label>
            ...
            <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                <button type="button" class="assign-btn text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Asignar</button>
                <button type="button" class="reject-request text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 focus:outline-none rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 focus:z-10">Rechazar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function(){
        ...
        $('.reject-request').click(function(){
            ...
            $.post('/../utilities/reject.php', data, function(res){
                modal.hide()
                location.reload()
            })
        })
        ...
        $('.assign-btn').click(function(){
            let request_id = $('#request_id').val()
            let comment = $('#message').val()
            let area_id = $('#sel-area').find(':selected').val()
            ...
            $.post('/../utilities/request.php', data, function(res){
                modal.hide()
                location.reload()
            })
        })
